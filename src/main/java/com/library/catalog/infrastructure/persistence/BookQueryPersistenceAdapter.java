...
public class BookQueryPersistenceAdapter implements BookQueryPersistencePort {
    ...
    public BookQueryPersistenceAdapter(BookRepository bookRepository, BookEntityMapper bookEntityMapper) {
    ...
    public Book findById(BookId bookId) {
       var book = bookRepository.findById(bookId.id()).orElseThrow(() -> new BookNotFoundException(bookId));
